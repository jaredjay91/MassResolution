...
void getWidthsOftnpFits_etappJpsi(int RD0MC1=1) {
  TString RDorMC;
  ...
  double etabins[5] = {0.0, 1.2, 1.8, 2.1, 2.4};
  const static int numbins = sizeof(etabins)/sizeof(double)-1;

  TH1D* hsigma1 = new TH1D("hsigma1","hsigma1",numbins,etabins);
  TH1D* hsigma2 = new TH1D("hsigma2","hsigma2",numbins,etabins);
  ...
  cout << "starting loop" << endl;
    for (int i=0; i<numbins; i++) {
      ...
      RooFitResult* fitRes = (RooFitResult*)f1->Get(Form("tpTree/Trg_absetadep/abseta_bin%i__pt_bin0__Glb_true__HybridSoftId_2018_true__InAcceptance_2018_Tight_true__TM_true__passedDXY_SOFT_true__passedDZ_SOFT_true__cbGausPassFailPol1/fitresults;1",i));
...
*/
  ...
  c1->SaveAs(Form("Results/etaMassResJpsipp%s.pdf",RDorMC.Data()));
  ...
  c2->SaveAs(Form("Results/etaMassScaleJpsipp%s.pdf",RDorMC.Data()));

  //Set up the output tree.
  TString outFileName = Form("Results/etaMassResJpsipp%s.root",RDorMC.Data());

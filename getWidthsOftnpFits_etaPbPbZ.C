...
void getWidthsOftnpFits_etaPbPbZ(int RD0MC1=1) {
  ...
  cout << "starting loop" << endl;
   for (int i=0; i<numbins; i++) {
     ...
     RooFitResult* fitRes = (RooFitResult*)f1->Get(Form("tpTree/Trg_absetadep/abseta_bin%i__pt_bin0__tag_hiBin_bin0__isTightMuon_true__BWConvCBExpFixedWidthBW/fitresults;1",i));
  ...
  c1->SaveAs(Form("Results/etaMassResZPbPb%s.pdf",RDorMC.Data()));
  ...
  c2->SaveAs(Form("Results/etaMassScaleZPbPb%s.pdf",RDorMC.Data()));

  //Set up the output tree.
  TString outFileName = Form("Results/etaMassResZPbPb%s.root",RDorMC.Data());

...
void getWidthsOftnpFits_etappZ(int RD0MC1=1) {
  TString RDorMC;
  ...
  cout << "starting loop" << endl;
   for (int i=0; i<numbins; i++) {
     ...
     RooFitResult* fitRes = (RooFitResult*)f1->Get(Form("tpTree/Trg_absetadep/abseta_bin%i__charge_bin0__combRelIsoPF04dBeta_bin0__pt_bin0__tag_abseta_bin0__tag_pt_bin0__TightId_true__tag_HLT_HIL3Mu12_true__BWResCBExp/fitresults;1",i));
  ...
  c1->SaveAs(Form("Results/etaMassResZpp%s.pdf",RDorMC.Data()));
  ...
  c2->SaveAs(Form("Results/etaMassScaleZpp%s.pdf",RDorMC.Data()));

  //Set up the output tree.
  TString outFileName = Form("Results/etaMassResZpp%s.root",RDorMC.Data());

...
void getWidthsOftnpFits_etapPbZ(int RD0MC1=1) {
  TString RDorMC;
  ...
  cout << "starting loop" << endl;
   for (int i=0; i<numbins; i++) {
     ...
     RooFitResult* fitRes = (RooFitResult*)f1->Get(Form("tpTree/MuId_absetadep/abseta_bin%i__pt_bin0__TightId_true__BWFixedResCBExp/fitresults;1",i));
  ...
  c1->SaveAs(Form("Results/etaMassResZpPb%s.pdf",RDorMC.Data()));
  ...
  c2->SaveAs(Form("Results/etaMassScaleZpPb%s.pdf",RDorMC.Data()));

  //Set up the output tree.
  TString outFileName = Form("Results/etaMassResZpPb%s.root",RDorMC.Data());
